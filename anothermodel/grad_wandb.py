...
class GradNormOptimizer:
    def __init__(self, model, num_tasks=2, alpha=1.5, weight_lr=1e-4):
        self.model = model
        ...
        self.optimizer_weights = torch.optim.Adam([self.weights], lr=weight_lr)
    ...
    def get_shared_parameters(self):
        ...
        # Assuming shared_representation is the last shared layer
    ...
    def update_weights(self, task_losses, shared_params):
        ...
        if self.initial_losses is None:
            ...
            return F.softmax(self.weights, dim=0)  # Return normalized weights
        
        # 1. Compute gradient norms G_W
        grad_norms = self.compute_gradient_norms(task_losses, shared_params)
        
        # 2. Calculate relative learning rates r_i(t)
        current_losses = torch.stack([loss.detach() for loss in task_losses])
        normalized_losses = current_losses / (self.initial_losses + 1e-8)  # Add epsilon for stability
        relative_rates = normalized_losses / torch.mean(normalized_losses)
        
        # 3. Calculate average gradient norm (as constant)
        avg_grad_norm = torch.mean(grad_norms).detach()
        
        # 4. Calculate targets for each task
        targets = avg_grad_norm * (relative_rates ** self.alpha)
        
        # 5. Calculate GradNorm loss
        L_grad = torch.sum(torch.abs(grad_norms - targets))
        ...
        # Return normalized weights for loss computation
...
class MultiTaskAlzheimerModel(pl.LightningModule):
    def __init__(self, 
        ...
        self.gradnorm_optimizer = GradNormOptimizer(
            model=self, 
            num_tasks=2, 
    ...
    def on_fit_start(self):
        self.gradnorm_optimizer.to(self.device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        shared_params = self.gradnorm_optimizer.get_shared_parameters()
        
        # Set initial losses for the first batch
        if not self.initial_batch_processed:
            self.gradnorm_optimizer.set_initial_losses(task_losses)
            self.initial_batch_processed = True
        
        # Update weights using GradNorm
        weights = self.gradnorm_optimizer.update_weights(task_losses, shared_params)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        weights = F.softmax(self.gradnorm_optimizer.weights, dim=0)
    ...
    def configure_optimizers(self):
        # Only optimize model parameters, not task weights (handled by GradNorm)
        model_params = [p for name, p in self.named_parameters() 
                       if 'gradnorm_optimizer.weights' not in name]
