...
class CombinedOptimizer:
    ...
    def update_weights(self, losses, shared_params):
        ...
        # Calculate gradient norms and their differences properly
        grad_norms = []
        for i, loss in enumerate(losses):
            grads = torch.autograd.grad(loss, shared_params, retain_graph=True)
            grad_norm = torch.norm(torch.stack([g.norm() for g in grads]))
        ...
        # Simple approach: calculate relative importance from loss values
        loss_values = torch.stack([loss.detach() for loss in losses])
        loss_weights = F.softmax(loss_values, dim=0)
        
        # Calculate gradient imbalance - how far are we from ideal balance?
        ...
        # Adaptive lambda that actually changes with gradient differences
        # Bound between 0.1 and 0.9 to prevent getting stuck at extremes
        delta_lambda = self.eta * grad_diff * 2.0  # Scale up for more movement
        
        # Add a minimum change threshold to prevent stagnation
        min_change = 0.01
        if delta_lambda < min_change:
            delta_lambda = torch.tensor(min_change)
        
        # Update lambda with direction based on which task needs more attention
...
class MultiTaskAlzheimerModel(pl.LightningModule):
    def __init__(self, 
        ...
        self.loss_history = {
            ...
            'regression': [],
            'weights': [],
            'lambda_values': [],  # Add this line to track lambda values
