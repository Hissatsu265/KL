...
class CrossModalFusionBlock(nn.Module):
    def __init__(self, input_dim=640, hidden_dim=256, output_dim=128):
        super().__init__()
...
class GradNormOptimizer:
    def __init__(self, model, num_tasks=2, alpha=1.5, temperature=3.0):
        self.model = model
        self.num_tasks = num_tasks
        self.alpha = alpha  # Trong DWA, alpha được dùng như temperature parameter
        self.temperature = temperature  # Thêm temperature parameter cho DWA
        self.initial_losses = None
        self.prev_losses = None  # DWA cần losses từ iteration trước
        
        # Khởi tạo weights ban đầu (uniform)
        a, b = 0.95, 0.05  # uniform beta
        sample = torch.distributions.Beta(a, b).sample((num_tasks,))
        sample = sample / sample.sum()  # ensure sum to 1
        self.task_weights = nn.Parameter(torch.log(sample), requires_grad=False)  # DWA không cần grad
        # ================================================================
        # DWA không cần optimizer cho weights vì weights được tính trực tiếp
    ...
    def compute_grad_norm_loss(self, losses, shared_params):
        # Trong DWA, ta không tính grad norm loss mà tính relative decrease rates
        if self.initial_losses is None:
            self.initial_losses = [loss.item() for loss in losses]
            self.prev_losses = [loss.item() for loss in losses]
            # Trả về 0 cho lần đầu tiên
            return torch.tensor(0.0, device=losses[0].device)
            
        # Tính relative decrease rates
        current_losses = [loss.item() for loss in losses]
        relative_rates = []
        
        for i in range(self.num_tasks):
            if self.prev_losses[i] != 0:
                rate = current_losses[i] / self.prev_losses[i]
            else:
                rate = 1.0
            relative_rates.append(rate)
        
        # Cập nhật prev_losses cho lần tính toán tiếp theo
        self.prev_losses = current_losses
        
        # Tính weights theo công thức DWA
        relative_rates = torch.tensor(relative_rates, device=losses[0].device)
        
        # Áp dụng temperature scaling
        exp_rates = torch.exp(relative_rates / self.alpha)  # alpha đóng vai trò như temperature
        weights = self.num_tasks * exp_rates / torch.sum(exp_rates)
        
        # Cập nhật task_weights
        self.task_weights.data = torch.log(weights + 1e-8)  # Thêm epsilon để tránh log(0)
        
        # Trả về 0 vì DWA không có loss cần optimize
        return torch.tensor(0.0, device=losses[0].device)
        
    def update_weights(self, losses, shared_params):
        # Tính toán weights mới theo DWA
        gradnorm_loss = self.compute_grad_norm_loss(losses, shared_params)
        
        # DWA không cần optimizer steps
...
class CombinedOptimizer:
    ...
    def update_weights(self, losses, shared_params):
        ...
        combined_weights = (self.lambda_param *  gn_weights)
